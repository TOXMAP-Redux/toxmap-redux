...
- **7.BUG.42: Census county hover overlaps TRI/Superfund popup** — County hover tooltip is now hidden when a TRI facility or Superfund site popup is open, preventing visual overlap. Added condition to check `!selectedFacilityId && !selectedSuperfundEpaId` before rendering the county tooltip. [agent]
...
- **7.BUG.39/40/41/42 + 7.UX.6 regression tests** — Added 13 E2E scenarios (`@7BUG39`, `@7BUG40`, `@7BUG41`, `@7BUG42`, `@7UX6`) in `ux_invariants.feature` for census demographics layer z-order, Census 2000 disabled buttons, historical color scheme, popup overlap prevention, and county hover tooltip. [agent]
