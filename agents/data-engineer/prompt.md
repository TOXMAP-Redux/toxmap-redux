...
| 1.5.6 | **⚠️ SUPERSEDED by ADR-005 (2026-07-27):** Basemap tiles are now served from [OpenFreeMap](https://openfreemap.org), not self-hosted PMTiles on R2. No data engineering work is required for the basemap. The `scripts/build_pmtiles.py` script and manual R2 upload are no longer needed. See [ADR-005](../docs/adr/ADR-005-openfreemap-basemap-tiles.md) for the decision record. |
