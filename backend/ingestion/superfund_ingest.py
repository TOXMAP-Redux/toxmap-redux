...
def _validate_url(url: str) -> str:
    ...
    if not any(url.startswith(prefix) for prefix in allowed_prefixes):
        raise ValueError(
            f"SSRF guard: URL {url!r} is not under allowed prefixes"
        )
    return url
    ...
    for status_code in NPL_STATUS_CODES:
        ...
        while True:
            try:
                ...
                logger.info("  SEMS %s: fetched %d records (mapped: %d)", 
                           status_code, len(data), len(epa_id_to_site_id))
                ...
                logger.warning("Failed to fetch SEMS sites (%s offset %d): %s", 
                             status_code, offset, exc)
                break
...
def _fetch_contaminants(site_ids: list[str]) -> dict[str, list[str]]:
    ...
    while True:
        try:
            ...
            logger.info("  Contaminants: fetched %d records (total: %d, sites with data: %d)",
                       len(data), total_fetched, len(site_contaminants))
...
def _download_superfund_arcgis() -> pd.DataFrame:
    ...
    # Fields to retrieve
    out_fields = (
        "EPA_ID,SITE_NAME,STREET_ADDR_TXT,CITY_NAME,STATE_CODE,"
        "COUNTY,ZIP_CODE,NPL_STATUS_CODE"
    )
...
def _ingest_superfund(
    ...
    """Upsert Superfund sites with contaminants. Returns number of rows processed."""
    rows = 0
    skipped = 0
...
def ingest_superfund(db_url: str, skip_contaminants: bool = False) -> None:
    ...
    with engine.begin() as conn:
        rows = _ingest_superfund(df, conn, epa_to_contaminants)
